defmodule Mix.Tasks.Firmware.Push do
  ...
  def run(argv) do
    ...
    :success = :ssh_connection.subsystem(connection_ref, channel_id, 'fwup', :infinity)

    chunks =
      File.open!(firmware_path, [:read])
      |> IO.binstream(16384)
  ...
  defp discover_firmware(opts) do
    target =
      opts[:target] || System.get_env("MIX_TARGET") ||
        ...
        Or
          $ mix firmware.push nerves.local --firmware path/to/app.fw
          $ mix firmware.push nerves.local --target rpi0
        """)
    ...
    _ = Application.stop(:mix)
    System.put_env("MIX_TARGET", target)
    _ = Application.start(:mix)
  ...
  defp target_ip_address_or_name_msg() do
    ...
    Example:
      ...
      If the hostname is `nerves@nerves.local`, you should use:
      `mix firmware.push nerves.local`
    """
  end
end
